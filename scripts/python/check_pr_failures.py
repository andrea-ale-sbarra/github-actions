...
"is this PR done?" cover marker on the Jira board), this script:
  ...
  2. Resolves the PR's merge commit SHA + the files it touched, via the
     GitHub API.
  3. Looks up the Acceptance build that ran on that SHA in Testray.
...
from utils.pr_check import check_pr_failures_for_routine, print_pr_check_standalone
...
def main():
    ...
    for routine in select_routines():
        try:
            results = check_pr_failures_for_routine(routine)
